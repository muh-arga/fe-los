...
const ApproveUser = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseURL}/api/users?status=0`, {
        headers: {
  ...
  const handleApprove = (id) => {
    Swal.fire({
      title: "Approve User?",
      text: "Apakah anda yakin ingin approve user ini?",
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        axios
          .put(`${baseURL}/api/users/approve/${id}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          .then((res) => {
            Swal.fire("Berhasil!", "User berhasil diapprove.", "success").then(
              () => {
                window.location.reload();
              }
            );
          ...
          .catch((err) => {
            console.log(err);
            Swal.fire("Gagal!", "User gagal diapprove.", "error").then(() => {
              window.location.reload();
            });
          });
      }
                ...
                <tr key={index}>
                  ...
                  <td>
                    <div className="d-flex justify-content-center align-items-center px-4">
                      <button
                        type="button"
                        className="btn btn-sm fw-medium btn-success text-light me-2 col-6"
                        onClick={() => handleApprove(item.id)}
                      >
                        <i className="bi bi-person-check-fill me-2 text-light"></i>
                        Approve
                      </button>
                      <button
                        type="button"
